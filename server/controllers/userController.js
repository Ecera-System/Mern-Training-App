...
exports.signup = async (req, res) => {
  try {
    ...
    if (findEmail)
      return res.status(406).json({ error: "This email already exists!" });
...
exports.resendCode = async (req, res) => {
  try {
    ...
    if (!findUser)
      return res.status(404).json({ error: "This email doesn't exists." });
    if (findUser.status === "active")
      return res.status(406).json({ error: "This email already activated!." });
...
exports.activateAccount = async (req, res) => {
  try {
    ...
    if (expirationTime < now)
      return res.status(408).json({ error: "Invalid verification code!" });
    if (Number(code) !== dbCode)
      return res.status(408).json({ error: "Wrong verification code!" });
...
exports.googleSignin = async (req, res) => {
  try {
    ...
    if (!email_verified)
      return res.status(401).send({ error: "Email isn't verified!" });
...
exports.updateUserPassword = async (req, res, next) => {
  ...
  if (oldPassword === newPassword) {
    return res
      .status(400)
      .json({ error: "Old Password and New password should be different" });
  }
