...
exports.getAllStudents = async (req, res, next) => {
  try {
    ...
    if (query) {
      ...
      const students = queriesStudent.filter(
...
exports.getAllUsers = async (req, res, next) => {
  try {
    ...
    if (query) {
      ...
      const users = profiles.filter((user) => user.userId !== null);
...
exports.updateUserStatusById = async (req, res, next) => {
  try {
    ...
    if (!result)
      return res.status(404).json({ error: "No user founded with this id." });

    res
      .status(200)
      .json({ success: "User status changed successfully!", data: result });
  } catch (error) {
    next(error);
  }
};
...
exports.deleteUserById = async (req, res, next) => {
  try {
    ...
    if (user.role === "admin")
      return res.status(403).json({ error: "Admin can't be deleted!" });
    ...
    res
      .status(200)
      .json({ success: "User deleted successfully!", data: "result" });
  } catch (error) {
    next(error);
  }
};
