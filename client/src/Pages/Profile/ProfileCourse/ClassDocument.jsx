...
const ClassDocument = ({ content, setContent }) => {
    ...
    <>
      ...
      <div className="mt-5 mb-28 w-full text-gray-600">
        <div className="p-5 bg-white border rounded-lg text-center relative">
          ...
          <h1 className="text-xl font-semibold mb-3">{content.title}</h1>
            ...
            <div className="flex items-start justify-between gap-7 mt-5">
              ...
              <div className="w-1/3">
                <ul className="list-none flex flex-col gap-4">
                  ...
                  {content.modules.map((item, mIndex) => (
                    <li
                      key={mIndex}
                    ...
                    >
                      <button
                        onClick={() =>
                          setCntIndex((pre) => (pre !== mIndex ? mIndex : null))
                        }
                        className="w-full flex items-center justify-between"
                      >
                        <h3 className="text-base font-medium text-violet-600 capitalize">
                          {item.title}
                        </h3>
                        <MdKeyboardArrowDown
                          className={`inline-block text-3xl duration-500 ${
                            cntIndex === mIndex ? "rotate-180" : "rotate-0"
                          }`}
                        />
                      </button>
                      <ul
                        className={`overflow-hidden flex flex-col gap-1 maxH-0 ${
                          cntIndex === mIndex ? "maxH-full mt-3" : ""
                        }`}
                      >
                        {item.videos.map((video, vIndex) => (
                          <li key={video._id}>
                            <button
                              className={`w-full pl-3 pr-5 py-3 rounded text-start flex gap-2 ${
                                moduleIndex === mIndex && videoIndex === vIndex
